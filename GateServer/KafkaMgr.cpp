...
bool KafkaMgr::init(const std::string& brokers, const std::string& group)
{
	...
	if (!_confGlobal || !_confTopic) return false;
...
bool KafkaMgr::produce(const std::string& topic, const std::string& key, const std::string& payload)
{
	...
	auto rc = _producer->produce(topic, RdKafka::Topic::PARTITION_UA, RdKafka::Producer::RK_MSG_COPY,
		(void*)payload.data(), payload.size(), &key, nullptr);
	if (rc != RdKafka::ERR_NO_ERROR) return false;
	_producer->poll(0);
	return true;
}
...
bool KafkaMgr::consume(const std::string& topic, std::function<void(const std::string& key, const std::string& payload)> cb)
{
	...
	if (_consumer->subscribe({ topic })) return false;
...
bool KafkaMgr::startConsumerLoop()
{
	while (_running.load()) {
		...
		if (msg->err() == RdKafka::ERR_NO_ERROR) {
			std::string key; if (msg->key()) key.assign(*msg->key());
...
void KafkaMgr::close()
{
	...
	_producer.reset(); _consumer.reset();
	_confGlobal.reset(); _confTopic.reset();
} 