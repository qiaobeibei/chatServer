    ...
    ui(new Ui::ChatPage)
{
    ...
    // 绑定呼叫与挂断按钮
    connect(ui->call_btn, &QPushButton::clicked, this, &ChatPage::on_call_btn_clicked);
    connect(ui->hangup_btn, &QPushButton::clicked, this, &ChatPage::on_hangup_btn_clicked);
}
...
void ChatPage::on_call_btn_clicked()
{
    if (!_user_info) return;
    auto self = UserMgr::GetInstance()->GetUserInfo();
    _peer_uid = _user_info->_uid;
    TcpMgr::GetInstance()->sendRtcCall(self->_uid, _peer_uid, QStringLiteral("audio"));
}

void ChatPage::on_hangup_btn_clicked()
{
    if (_peer_uid == 0) return;
    auto self = UserMgr::GetInstance()->GetUserInfo();
    TcpMgr::GetInstance()->sendRtcHangup(self->_uid, _peer_uid);
    _peer_uid = 0;
}
