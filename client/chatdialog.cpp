    ...
    _state(ChatUIMode::ChatMode),_last_widget(nullptr),_cur_chat_uid(0)
{
    ...
    // 连接 RTC 信令
    connect(TcpMgr::GetInstance().get(), &TcpMgr::sig_rtc_call, this, &ChatDialog::slot_rtc_call);
    connect(TcpMgr::GetInstance().get(), &TcpMgr::sig_rtc_call_rsp, this, &ChatDialog::slot_rtc_call_rsp);
    connect(TcpMgr::GetInstance().get(), &TcpMgr::sig_rtc_offer, this, &ChatDialog::slot_rtc_offer);
    connect(TcpMgr::GetInstance().get(), &TcpMgr::sig_rtc_answer, this, &ChatDialog::slot_rtc_answer);
    connect(TcpMgr::GetInstance().get(), &TcpMgr::sig_rtc_ice, this, &ChatDialog::slot_rtc_ice);
    connect(TcpMgr::GetInstance().get(), &TcpMgr::sig_rtc_hangup, this, &ChatDialog::slot_rtc_hangup);

    // 嵌入 WebRTC 视图
    _rtcBridge = new RtcBridge(this);
    _rtcBridge->loadPage(QUrl(QStringLiteral("qrc:/static/webrtc.html")));
    // 将视图放入标题区域下方（示例：放入 chat_data_wid 顶部）
    auto layout = qobject_cast<QVBoxLayout*>(ui->verticalLayout_2);
    if (layout) {
        layout->insertWidget(1, _rtcBridge->view(), 0);
    }

    // 本地 SDP/ICE 通过 TcpMgr 发送
    connect(_rtcBridge, &RtcBridge::sig_local_answer, this, [this](QJsonObject obj){
        if (_peer_uid == 0) return;
        auto self = UserMgr::GetInstance()->GetUserInfo();
        QJsonObject payload; payload["from"] = self->_uid; payload["to"] = _peer_uid; payload["type"] = obj.value("type").toString(); payload["sdp"] = obj.value("sdp").toString();
        QJsonDocument doc(payload);
        emit TcpMgr::GetInstance()->sig_send_data(ReqId::ID_RTC_ANSWER, doc.toJson(QJsonDocument::Compact));
    });
    connect(_rtcBridge, &RtcBridge::sig_local_ice, this, [this](QJsonObject obj){
        if (_peer_uid == 0) return;
        auto self = UserMgr::GetInstance()->GetUserInfo();
        QJsonObject payload; payload["from"] = self->_uid; payload["to"] = _peer_uid; payload["candidate"] = obj;
        QJsonDocument doc(payload);
        emit TcpMgr::GetInstance()->sig_send_data(ReqId::ID_RTC_ICE, doc.toJson(QJsonDocument::Compact));
    });
}
...
void ChatDialog::slot_rtc_call(QJsonObject obj)
{
    // 收到来电
    int from = obj.value("from").toInt();
    QString media = obj.value("type").toString();
    if (_in_call) {
        // 忙线：忽略或提示
        QMessageBox::information(this, tr("来电"), tr("忙线中，来自 %1 的%2来电").arg(from).arg(media));
        return;
    }
    auto ret = QMessageBox::question(this, tr("来电"), tr("来自 %1 的%2通话，是否接听？").arg(from).arg(media), QMessageBox::Yes|QMessageBox::No);
    if (ret == QMessageBox::Yes) {
        _in_call = true; _peer_uid = from;
        // 等待 onRemoteOffer -> rtc.js 生成本地 answer 后通过 sig_local_answer 发送
    } else {
        // 直接挂断通知
        TcpMgr::GetInstance()->sendRtcHangup(UserMgr::GetInstance()->GetUserInfo()->_uid, from);
    }
}

void ChatDialog::slot_rtc_call_rsp(QJsonObject obj)
{
    int err = obj.value("error").toInt();
    int to = obj.value("to").toInt();
    if (err != 0) {
        QString msg = obj.value("msg").toString();
        QMessageBox::warning(this, tr("呼叫失败"), tr("对 %1 呼叫失败: %2").arg(to).arg(msg));
        _in_call = false; _peer_uid = 0;
        return;
    }
    _in_call = true; _peer_uid = to;
    QMessageBox::information(this, tr("呼叫中"), tr("正在呼叫 %1 ...").arg(to));
}

void ChatDialog::slot_rtc_offer(QJsonObject obj)
{
    // 转给 web 引擎
    if (!_rtcBridge) return;
    _rtcBridge->sendOffer(obj);
}

void ChatDialog::slot_rtc_answer(QJsonObject obj)
{
    if (!_rtcBridge) return;
    _rtcBridge->sendAnswer(obj);
}

void ChatDialog::slot_rtc_ice(QJsonObject obj)
{
    if (!_rtcBridge) return;
    _rtcBridge->sendIce(obj);
}

void ChatDialog::slot_rtc_hangup(QJsonObject obj)
{
    int from = obj.value("from").toInt();
    if (_in_call && (_peer_uid == from || _peer_uid == 0)) {
        _in_call = false; _peer_uid = 0;
        QMessageBox::information(this, tr("通话结束"), tr("与 %1 的通话已结束").arg(from));
    }
}
